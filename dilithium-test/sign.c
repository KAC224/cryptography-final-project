...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
    // large arrays moved to static to avoid stack overflow
    static uint8_t seedbuf[2*SEEDBYTES + CRHBYTES];
    static uint8_t tr[TRBYTES];
    static polyvecl mat[K];
    static polyvecl s1, s1hat;
    static polyveck s2, t1, t0;

    // pointers to parts of seedbuf
    const uint8_t *rho = seedbuf;
    const uint8_t *rhoprime = rho + SEEDBYTES;
    const uint8_t *key = rhoprime + CRHBYTES;
